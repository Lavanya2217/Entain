WITH convs AS (  SELECT *, CASE WHEN Conversion IN ('FTD', 'Registration') THEN 'cpa' ELSE 'rev_share' END AS join_type
                 ...
                 WHERE Date >= '2020-01-01' AND REGEXP_CONTAINS(ChannelGrouping, '(?i)aff') ),
      
      aff AS (   SELECT * FROM {{ref('costs_affiliates')}}
                 WHERE Date_aff >= '2020-01-01' AND REGEXP_CONTAINS(Brand_aff, '(?i)ladb|coral') ), 
...
SELECT  IFNULL(Date, date_aff)            AS Date
      ...
      -- ,''                                AS Campaign
       ...
       ,'' AS partner_name
       ,IFNULL(GA_wm_track, 
               CAST(tracker_id AS INT64)) AS cid

WITH convs AS (  SELECT ul.*, beneficiary_id, CASE WHEN Conversion IN ('FTD', 'Registration') THEN 'cpa' ELSE 'rev_share' END AS join_type
                 FROM {{ref('gvc_conversions_unique_ledger')}} ul
      ...
      aff AS (   SELECT * FROM {{ref('costs_affiliates')}}
                 WHERE Date_aff >= '2021-01-01' AND REGEXP_CONTAINS(Brand_aff, '(?i)bwin|gala|cheeky|foxy') ), 

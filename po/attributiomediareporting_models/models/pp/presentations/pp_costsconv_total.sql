WITH sess AS ( SELECT distinct * REPLACE(CAST(date AS DATE) AS Date) FROM {{ref('pp_sessions_count_cids')}})
...
UNION ALL
    ...
    FROM {{ref('pp_costsconv_google_ppc')}} p
...
UNION ALL
    ...
    FROM {{ref('pp_costsconv_nocids')}} n

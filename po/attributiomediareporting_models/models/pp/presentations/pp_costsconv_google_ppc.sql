WITH  ffr  AS ( SELECT *
                FROM {{ref('pp_dim_campaigns')}} WHERE CAST(Date AS Date) IS NOT NULL AND newest_rank =1),
     ...
     FINAL AS ( SELECT c.* REPLACE (CASE WHEN SAFE_CAST(campaign AS INT64) IS NOT NULL AND d.campaign_name IS NOT NULL
                ...
                FROM (SELECT * REPLACE(CASE WHEN campaignid IS NOT NULL THEN CAST(campaignid AS STRING)
        ...
        lal AS ( SELECT distinct Date, Brand, ChannelGrouping, Publisher, campaign_id
                 ...
                 FROM(   SELECT * REPLACE(CAST(Date AS DATE) AS Date),
                         ...
                         WHERE Date >= '2021-01-01' AND google_excl = 1)
                 GROUP BY 1,2,3,4,5
                 ),
         ...
         bb AS ( SELECT distinct aa.* REPLACE( CASE WHEN campaignid IS NULL AND aa.cid IS NOT NULL AND campaign_name IS NOT NULL
                                                    ...
                                                    END AS campaignid,
                                               CASE WHEN tt.channelgrouping <> aa.channelgrouping THEN tt.channelgrouping
                                                    ELSE aa.channelgrouping END AS channelgrouping )
                 FROM final aa
                 LEFT JOIN (SELECT distinct *
                            FROM {{ref('pp_dim_campaigns')}} )tt
                 ON CAST(aa.campaignid AS STRING) = tt.campaign_id AND aa.date BETWEEN tt.date AND end_date
                 ),
 ...
 rr         AS ( SELECT a.* REPLACE(CASE WHEN a.campaignid IS NULL AND cpid IS NOT NULL THEN cpid ELSE a.campaignId END AS campaignId)
    ...
    aa      AS ( SELECT  Date                                                                                       AS Date
                       ...
                       ,ROUND(SUM(IF(Conversion = 'FTD'     AND lag_hours <= 72     , event_value_GBP, 0)),0)       AS FTDs_Value_72h
                       ,SUM(IF(Conversion = 'Deposit'       AND lag_hours <= 72, 1, 0))                             AS Deposits_72h
                       ,ROUND(SUM(IF(Conversion = 'Deposit' AND lag_hours <= 72     , event_value_GBP, 0)),0)       AS Deposits_Value_72h
                       ,SUM(IF(Conversion = 'Bet'           AND lag_hours <= 72, 1, 0))                             AS Bets_72h
                       ,ROUND(SUM(IF(Conversion = 'Bet'     AND lag_hours <= 72     , event_value_GBP, 0)),0)       AS Bets_Value_72h

WITH ru AS (SELECT * REPLACE(CASE WHEN REGEXP_CONTAINS(Campaign, 'c:') THEN REGEXP_REPLACE(Campaign, '-', '|') ELSE Campaign END AS Campaign,
            ...
            FROM {{ref('gvc_costsconv_total')}}  WHERE DATE < CURRENT_DATE()),


   vu AS   (SELECT * --REPLACE(CASE WHEN (NOT REGEXP_CONTAINS(ChannelGrouping, '(?i)crm|ref|orga|affi|ispaly|partner|ppc|display') OR ChannelGrouping = 'Other') AND Spend IS NULL
            ...
            FROM(
                           ...
                           ,CASE WHEN REGEXP_CONTAINS(ru.Campaign, 'cid:|c:') THEN
                            CASE WHEN REGEXP_CONTAINS(SPLIT(ru.Campaign, '|')[SAFE_OFFSET(13)], 'cid') THEN 'Src'
                                 WHEN REGEXP_CONTAINS(SPLIT(ru.Campaign, '|')[SAFE_OFFSET(15)], 'c:')  THEN 'Nsr' ELSE 'Reg' END
                                  END AS Split
                    FROM ru
                    LEFT JOIN (SELECT distinct * FROM {{ref('gvc_sessions_count')}} ) s
              ...
              UNION ALL
                    ...
                    WHERE REGEXP_CONTAINS(ChannelGrouping, '(?i)CRM') AND NOT REGEXP_CONTAINS(ChannelGrouping, '(?i)Social')
              ...
              UNION ALL
                    ...
                    FROM ru
                    LEFT JOIN (SELECT distinct * FROM {{ref('gvc_sessions_count')}} ) s
                    ON s.Brand = ru.Brand AND s.cp = ru.ChannelGrouping AND s.Date = ru.Date AND LOWER(s.source) = LOWER(ru.Publisher)
              ...
              UNION ALL
                    ...
                    FROM ru
                    LEFT JOIN (SELECT distinct * FROM {{ref('gvc_sessions_count')}} ) s
                    ON s.Brand = ru.Brand AND s.cp = ru.ChannelGrouping AND s.Date = ru.Date AND LOWER(s.source) = LOWER(ru.Publisher) AND s.country = ru.country
   ...
   tu AS   (SELECT a.* REPLACE(b.string_field_2 AS crm_country
            ...
            FROM( SELECT *, CASE WHEN crm_conv = 1 THEN SUBSTR(SPLIT(REGEXP_REPLACE(campaign, '/', ''), '-')[SAFE_OFFSET(0)],-5,2) END AS crm_country
                  ...
                  FROM( SELECT *, CASE WHEN REGEXP_CONTAINS(Brand, '(?i)Bwin') AND REGEXP_CONTAINS(ChannelGrouping, '(?i)crm') AND NOT REGEXP_CONTAINS(Campaign, 'c:')
